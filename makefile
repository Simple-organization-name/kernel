...
OVMF_PATH 				= OVMF.fd
EMUL_ARGS				= -net none -serial file:serial.log -no-reboot -no-shutdown -d guest_errors -M accel=tcg,smm=off -d int #-cpu host -enable-kvm
...
setup-arch:
	sudo pacman -Syu
	sudo pacman -S xorriso qemu-desktop mingw-w64-gcc nasm
